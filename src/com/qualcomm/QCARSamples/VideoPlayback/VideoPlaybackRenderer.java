...
public class VideoPlaybackRenderer implements GLSurfaceView.Renderer
{
    ...
    private boolean mNewTrack[]                         = null;
    ...
    public VideoPlaybackRenderer() {
        ...
        mNewTrack = new boolean[VideoPlayback.NUM_TARGETS];
...
*/
        for (int i = 0; i < VideoPlayback.NUM_TARGETS; i++)
        {
        	//Ask whether the target currently being tracked and if so react to it
        	if (isTracking(i))
        	{
        		//if it is tracking reset the timestamp for lost tracking
        		// mLostTrackingSince[i] = -1;
        		...
        		if(mVideoPlayerHelper[i] != null)
        		{
        			//mNewTrack is used to define if the marker was out of camera
        			//in this case if the video was paused at the end before,
